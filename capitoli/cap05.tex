...
\begin{table}[H] % Usa [H] per renderla non flottante
    ...
    \end{tabular}
\end{table}
per esempio si usa per ogni livello di grigio il numero massimo rappresentabile del livello di grigio più alto ($255$), che è $8$ ($\log_{2}{(256)}$ ($[0, \, 255]$)). A questo punto, il
...
\begin{itemize}
    ...
    \item l'informazione di più messaggi indipendenti sono la somma delle informazione corrispettive.
\end{itemize}
...
perdere mantenendo ancora l'informazione accettabile. In particolare, si considerano due tipologie di criteri di fedeltà.\\
Il primo criterio è quello soggettivo, che si basa solamente sulla comparazione visiva tra due immagini, classificando le immagini in ranking da eccellente ad inutilizzabile.\\
Il secondo ed ultimo criterio è quello oggettivo, che si basa su delle metriche matematiche, che dipendono dall'errore ($\epsilon(x,y)$) e dall'errore assoluto ($\epsilon$),
...
Il modello di compressione delle immagini consiste nelle seguenti operazioni:
\begin{enumerate}
    ...
    \item codifica di simbolo, che esegue un sistema di codifica dell'immagine (operazione reversibile) e invia il risultato al canale;
    \item decodifica di simbolo, che prende l'immagine codificata dal canale e la ritrasforma a quella prima della codifica;
    \item mappatura inversa, che riporta l'immagine come matrice di pixel, leggermente diversa a causa della quantizzazione.
\end{enumerate}
\begin{figure}[htbp]
        \centering
        ...
        \label{fig:modello}
\end{figure}
A seconda dell'errore, la tipologia di compressione di un'immagine si suddivide in due grandi categorie:
\begin{itemize}
    \item compressione lossless, in cui l'errore è nullo, perciò non viene persa alcuna informazione, sfruttando la ridondanza di codifica ed interpixel;
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item $B \rightarrow [0,4; 0,9[$ (intervallo da considerare)
    ...
    \begin{itemize}
        ...
        \item $B \rightarrow 0,5\times0,5=0,25 \rightarrow [0,6; 0,6+0,25[ \rightarrow [0,6; 0,85[$ (intervallo da considerare)
    ...
    \begin{itemize}
        ...
        \item $B \rightarrow 0,5\times0,25=0,125 \rightarrow [0,7; 0,7+0,125[ \rightarrow [0,6; 0,825[$ 
...
tali compressioni non presentano delle perdite significative nella maggior parte degli ambiti in cui si usano. Lo standard più
