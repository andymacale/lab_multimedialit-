...
\begin{figure}[htbp]
        \centering
        \includegraphics[width=1\textwidth]{cap04/asse_tempo} 
        \caption{Variazione delle frequenze nell'asse temporale da $t_0=0 \,\text{s}$ a $t_3=1 \,\text{s}$} 
        \label{fig:asse_tempo}
\end{figure}
Come si può osservare:
\begin{itemize}
    \item da $t_0=0 \,\text{s}$ a $t_1=0,3 \,\text{s}$ si hanno le basse frequenze, in cui il segnale oscilla molto lentamente;
    \item da $t_1=0,3 \,\text{s}$ a $t_2=0,85 \,\text{s}$ si hanno le medie frequenze, dove le oscillazioni diventano sempre più ravvicinate e veloci;
    \item da $t_2=0,85 \,\text{s}$ a $t_3=1 \,\text{s}$ si hanno le alte frequenze, dove il segnale oscilla molto velocemente.
\end{itemize}
...
\begin{figure}[htbp]
        \centering
        \includegraphics[width=0.9\textwidth]{cap04/problemi_fourier.png} 
...
\begin{minipage}{0.46\textwidth} % Colonna Sinistra (48% della larghezza del testo)
    \begin{enumerate}
    \item definire una finestra di tempo di una dimensione fissa;
    \item applicare la trasformata di Fourier al segnale finestrato;
    \item memorizzare le frequenze trovate in quel intervallo di tempo;
    \item scorrere la finestra leggermente avanti nel tempo e si ripete le analisi, finché non finisce il segnale, costruendo una mappa.
\end{enumerate}
\end{minipage}
...
\begin{minipage}[c]{0.5\textwidth} % Colonna Destra (48% della larghezza del testo)
    \centering
        \centering
        \includegraphics[width=1\textwidth]{cap04/stft} 
...
\begin{figure}[htbp]
        \centering
        \includegraphics[width=1\textwidth]{cap04/wavelet} 
...
\begin{itemize}
    \item localizzazione simultanea spaziale e temporale, in cui la localizzazione della wavelet permette esplicitamente di rappresentare gli eventi nel tempo e la forma delle wavelet permette di rappresentare i dettagli differenti e la risoluzione differente;
...
\begin{figure}[htbp]
        \centering
        \includegraphics[width=1\textwidth]{cap04/serie} 
        \caption{Analisi dei dettagli delle serie} 
        \label{fig:serie}
\end{figure}
\subsection{Creazione della piramide dell'immagine}
...
\begin{figure}[htbp]
        \centering
        \includegraphics[width=0.8\textwidth]{cap04/Slide2} 
...
\begin{figure}[htbp]
        \centering
        \includegraphics[width=0.8\textwidth]{cap04/Slide3} 
        \caption{Schema di costruzione della piramide dell'immagine} 
        \label{fig:cost}
\end{figure}
\subsection{Rappresentazione wavelet e condizioni}
...
\begin{equation*}
   W_j=V_{j+1}-V_j \leftrightarrow V_{j+1}=V_j+W_j
\end{equation*}
...
\begin{equation*}
    f(x)=\begin{dcases}
        8 & \text{se } 0 \le x < 1/4 \\
        2 & \text{se } 1/4 \le x < 1/2 \\
        4 & \text{se } 1/2 \le x < 3/4 \\
        6 & \text{se } 3/4 \le x < 1
    \end{dcases}
\end{equation*}
Per prima cosa, si espande in serie in approssimazione in alta risoluzione per $j=2$.
\begin{center}
    $\varphi_{2,k}(x)=2\varphi\left(4x-k\right)=2\operatorname{rect}\left(4x-k-\frac{1}{2}\right)=2$ (altezza $2$)
\end{center}
\begin{itemize}
    \item $k=0$: $8 \,c_{2, 0}=2 \leftrightarrow c_{2, 0} = 4$;
    \item $k=1$: $2 \, c_{2, 1}=2 \leftrightarrow c_{2, 1} = 1$;
    \item $k=2$: $4 \,c_{2, 2}=2 \leftrightarrow c_{2, 2} = 2$;
    \item $k=3$: $6 \,c_{2, 3}=2 \leftrightarrow c_{2, 1} = 3$.
\end{itemize}
\begin{equation*}
    f(x)=4\varphi_{2,0}(x)+\varphi_{2,1}(x)+2\varphi_{2,2}(x)+6\varphi_{2,3}(x)
\end{equation*}
Tuttavia, tale struttura non dice nulla sulla struttura dell'approssimazione e sul dettaglio, perciò si riscrive la formula $V_2=V_1+W_1=V_0+W_0+W_1$.
\begin{itemize}
    \item $V_0 \rightarrow \varphi_{0,0}$;
    \item $W_0 \rightarrow \psi_{0,0}$;
    \item $W_1 \rightarrow \psi_{1,0}, \psi_{1,1}$.
\end{itemize}
Per quando riguarda i coefficienti, si calcolano nel modo seguente.
\begin{center}
\begin{minipage}{0.48\textwidth} % Colonna Sinistra (48% della larghezza del testo)
    \centering
    \begin{equation*}
        c_{j, k} = \frac{A+B}{\sqrt{2}}
    \end{equation*}
\end{minipage}
\hfill % Spazio elastico per separare le due colonne
\begin{minipage}{0.48\textwidth} % Colonna Destra (48% della larghezza del testo)
    \centering
    \begin{equation*}
        d_{j, k} = \frac{A-B}{\sqrt{2}}
    \end{equation*}
\end{minipage}
\end{center}
Da $V_2=V_1+W_1$:
\begin{itemize}
    \item $c_{1, 0}=\frac{c_{2, 0} + c_{2, 0}}{\sqrt{2}}=\frac{4+1}{\sqrt{2}}=\frac{5}{\sqrt{2}}$ (approssimazione);
    \item $c_{1, 1}=\frac{c_{2, 2} + c_{2, 3}}{\sqrt{2}}=\frac{2+3}{\sqrt{2}}=\frac{5}{\sqrt{2}}$ (approssimazione);
    \item $d_{1, 0}=\frac{c_{2, 0} + c_{2, 0}}{\sqrt{2}}=\frac{4-1}{\sqrt{2}}=\frac{3}{\sqrt{2}}$ (dettaglio);
    \item $d_{1, 1}=\frac{c_{2, 2} + c_{2, 3}}{\sqrt{2}}=\frac{2-3}{\sqrt{2}}=-\frac{1}{\sqrt{2}}$ (dettaglio).
\end{itemize}
Da $V_1=V_0+W_0$:
\begin{itemize}
    \item $c_{0, 0}=\frac{c_{1, 0} + c_{1, 1}}{\sqrt{2}}=(\frac{5}{\sqrt{2}}+\frac{5}{\sqrt{2}})\frac{1}{\sqrt{2}}=5$;
    \item $d_{0,0}=\frac{c_{1, 0} -c_{1, 1}}{\sqrt{2}}=(\frac{5}{\sqrt{2}}-\frac{5}{\sqrt{2}})\frac{1}{\sqrt{2}}=0$.
\end{itemize}
...
\end{equation*}
\subsection{Codifica di sottobanda}
La codifica di sottobanda consiste nel calcolare i coefficienti della bassa risoluzione da quelli di alta risoluzione, attraverso un algoritmo a struttura ad albero:
in particolare si sfrutta un filtro passa-basso ed un filtro passa-alto. Ciò si applica sia alle righe che alle colonne, come mostrato di seguito.
\begin{figure}[htbp]
        \centering
        \includegraphics[width=0.5\textwidth]{cap04/haar_transform} 
        \caption{Processo di codifica di sottobanda}
        \label{fig:haar_transform}
\end{figure}
In particolare:
\begin{itemize}
    \item $a(m, n)$ rappresenta l'approssimazione;
    \item $d^V(m, n)$ rappresentano i dettagli verticali di sottobanda;
    \item $d^H(m, n)$ rappresentano i dettagli orizzontali di sottobanda;
    \item $d^D(m, n)$ rappresentano i dettagli diagonali di sottobanda.
\end{itemize}
Ovviamente, una o più di queste sottobande può a loro volta essere suddivise in quattro sottobande più piccole e così via.
\subsection{Vantaggi della trasformata di Haar e trasformata Wavelet 2D}
La trasformata di Haar presenta i seguenti vantaggi:
\begin{itemize}
    \item contiene gli stessi pixel dell'immagine originale;
    \item le sue statistiche locali rimangono relativamente costanti e facilmente modellate;
    \item molti valori sono nulli od infinitesimi, perciò è un'ottima candidata per essere utilizzata nel processo di compressione (trattato nel prossimo capitolo);
    \item si possono estrarre sia le approssimazioni grossolane sia quelle fini.
\end{itemize}
Per questo motivo, si estende ciò alla trasformata Wavelet in due dimensioni, dove i filtri passa-basso e passa-alto sono definiti 
rispettivamente con $\varphi$ e $\psi$:
\begin{center}
\begin{minipage}{0.48\textwidth} % Colonna Sinistra (48% della larghezza del testo)
    \centering
    \begin{itemize}
    \item approssimazione: $\varphi(x, y)=\varphi(x)\varphi(y)$;
    \item dettagli verticali: $\psi^V(x, y)=\varphi(x)\psi(y)$;
    \item dettagli orizzontali: $\psi^H(x,y)=\psi(x)\varphi(y)$;
    \item dettagli diagonali: $\psi^D(x,y)=\psi(x)\psi(y)$.
    \end{itemize}
    \end{minipage}
\hfill % Spazio elastico per separare le due colonne
\begin{minipage}[c]{0.48\textwidth} % Colonna Destra (48% della larghezza del testo)
        \centering
        \includegraphics[width=0.8\textwidth]{cap04/2D} 
        \captionof{figure}{Esempio di trasformata Wavelet 2D} 
        \label{fig:wavelet_2D}
\end{minipage}
\end{center}
\subsection{Elaborazione dell'immagini nel dominio wavelet}
Nel dominio wavelet, l'elaborazione delle immagine avviene nel modo seguente:
\begin{enumerate}
    \item decomporre l'immagine nel dominio walet scegliendo una wavelet madre (solitamente Haar) e il numero di livello di scala di decomposizione;
    \item modificare i coefficienti wavelet a seconda della tipologia di elaborazione, come riduzione del rumore e compressione;
    \item ricostruire l'immagine con i coefficienti wavelet modificati.
\end{enumerate}
\vfill