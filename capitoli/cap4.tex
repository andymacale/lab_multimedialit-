...
\begin{equation*}
    V_{j+1}=V_j+W_j
\end{equation*}
Infine, si sfruttano $\phi(t)$ per $V_j$ e $\psi(t)$ per $W_j$, per ricostruire la decomposizione come coppia di wavelet:
\begin{equation*}
    f(t)=\sum_{k}c_k\phi\left(2^jt-k\right)+\sum_{k}d_{jk}\psi\left(2^jt-k\right)
\end{equation*}
dove:
\begin{itemize}
    \item il primo fattore indicano le basse risoluzioni;
    \item il secondo fattore indicano le alte risoluzioni.
\end{itemize}
\section{Trasformata di Haar}